...
- 聊天加密：云端聊天使用客户端与服务器之间的加密；秘密聊天额外采用端到端加密，且仅能在发起聊天的设备上访问。加密不等于账号或设备绝对安全。参阅 [官方加密说明](https://telegram.org/faq#q-so-how-do-you-encrypt-data)。
...
- 云端同步与缓存管理：云端聊天可在多台设备间同步，但应用和已下载媒体仍会占用本机存储空间。可在“设置 → 数据与存储 → 存储用量”中清理缓存，无需退出账号；秘密聊天不属于云端同步范围。参阅 [官方缓存清理说明](https://telegram.org/faq#q-how-do-i-log-out)。
